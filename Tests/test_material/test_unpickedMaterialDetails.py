...
class TestItemPage:
    ...
    def setup(self, login_to_item):
        ...
        self.req_input_xpath_list = [
            ...
            '//div[span[text()=" 需求日期 "]]/following-sibling::div//input',
            '//div[span[text()=" 需求数量 "]]/following-sibling::div//input',
            '//div[span[text()=" 父项料号 "]]/following-sibling::div//i',
            '//div[span[text()=" 子项料号 "]]/following-sibling::div//i',
        ]
        ...
        self.all_input_xpath_list = [
            '//div[span[text()=" 需求来源编码 "]]/following-sibling::div//input',
            '//div[span[text()=" 订单代码 "]]/following-sibling::div//input',
            '//div[span[text()=" 父项料号 "]]/following-sibling::div//input',
            '//div[span[text()=" 父项单位 "]]/following-sibling::div//input',
            '//div[span[text()=" 子项料号 "]]/following-sibling::div//input',
            '//div[span[text()=" 子项单位 "]]/following-sibling::div//input',
        ...
        self.all_num_input_xpath_list = [
            '//div[span[text()=" 需求数量 "]]/following-sibling::div//input',
            '//div[span[text()=" 未领料数量 "]]/following-sibling::div//input',
            '//div[span[text()=" 已发料数量 "]]/following-sibling::div//input',
        ...
        self.all_date_xpath_list = [
            '//div[span[text()=" 需求日期 "]]/following-sibling::div//input',
            '//div[span[text()=" 发料日期 "]]/following-sibling::div//input',
        ]
    ...
    def test_unpickedMaterialDetails_addsuccess(self, login_to_item):
        ...
        sleep(1)
        self.item.batch_modify_dialog_boxs(self.req_input_xpath_list[-2:],
                                           '(//table[@class="vxe-table--body"]//tr[1]/td[2])[2]')
        self.item.batch_modify_inputs(self.req_input_xpath_list[:2], text_str)
        self.item.batch_modify_inputs([self.req_input_xpath_list[2]], date_str)
        self.item.batch_modify_inputs([self.req_input_xpath_list[3]], '100')
    ...
    def test_unpickedMaterialDetails_delcancel(self, login_to_item):
        ...
        itemdata = self.item.get_find_element_xpath(
            f'//tr[./td[2][.//span[text()="{text_str}"]]]/td[2]'
        ).text
    ...
    def test_unpickedMaterialDetails_editcodesuccess(self, login_to_item):
        ...
        # 选中刚刚新增的测试数据
        self.item.click_button(f'//tr[./td[2][.//span[text()="1测试数据B"]]]/td[2]')
        # 点击修改按钮
    ...
    def test_unpickedMaterialDetails_editrepeat(self, login_to_item):
        ...
        # 选中1测试A工厂代码
        self.item.click_button('//tr[./td[2][.//span[text()="1测试数据C"]]]/td[2]')
        # 点击修改按钮
        self.item.click_edi_button()
    ...
    def test_unpickedMaterialDetails_delsuccess1(self, login_to_item):
        ...
        itemdata = self.driver.find_elements(
            By.XPATH, '//tr[./td[2][.//span[text()="1测试数据C"]]]/td[2]'
        )
    ...
    def test_unpickedMaterialDetails_editnamesuccess(self, login_to_item):
        ...
        # 选中编辑数据
        self.item.click_button('//tr[./td[2][.//span[text()="1测试数据A"]]]/td[2]')
        # 点击修改按钮
        self.item.click_edi_button()
        ...
        assert before_all_value == after_all_value and username == DateDriver().username and today_str in updatatime and int(
            num) == (int(len_num) + int(all_click) + 2) and after_click_class == before_click_class
    ...
    @allure.story("新增多条数据")
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_adds(self, login_to_item):
        self.item.click_add_button()  # 检查点击添加
        # 输入框要修改的值
        text_str = "1测试数据1"
        date_str = "2025/07/25 00:00:00"
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_select2(self, login_to_item):
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_select3(self, login_to_item):
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_select4(self, login_to_item):
        name = self.item.get_find_element_xpath(
            '//div[@class="vxe-table--body-wrapper body--wrapper"]/table[@class="vxe-table--body"]//tr[2]//td[2]'
        ).get_attribute('innerText')
        first_char = name[:1] if name else ""
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_select5(self, login_to_item):
        name = self.item.get_find_element_xpath(
            '//div[@class="vxe-table--body-wrapper body--wrapper"]/table[@class="vxe-table--body"]//tr[2]//td[2]'
        ).get_attribute('innerText')
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_clear(self, login_to_item):
        name = "3"
        sleep(1)
        self.item.click_button('//div[div[span[text()=" 需求来源编码"]]]//i[contains(@class,"suffixIcon")]')
        self.item.hover("包含")
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_ctrlIrepeat(self, login_to_item):
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_ctrlI(self, login_to_item):
        self.item.click_button('//table[@class="vxe-table--body"]//tr[2]//td[2]')
        ActionChains(self.driver).key_down(Keys.CONTROL).send_keys('i').key_up(Keys.CONTROL).perform()
        self.item.click_button('(//table[@class="vxe-table--body"]//tr[1]/td[2])[2]')
        self.item.enter_texts('(//table[@class="vxe-table--body"]//tr[1]/td[4])[2]//input', '1没有数据添加')
        sleep(1)
        ...
        self.item.click_button('//div[@class="vxe-modal--footer"]//span[text()="确定"]')
        self.item.get_find_message()
        self.item.select_input('父项料号', '1没有数据添加')
        ele2 = self.item.get_find_element_xpath('(//table[@class="vxe-table--body"]//tr[1]/td[4])[1]').get_attribute(
            "innerText")
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_ctrlM(self, login_to_item):
        ...
        self.item.click_button('(//table[@class="vxe-table--body"]//tr[1]/td[2])[2]')
        self.item.enter_texts('(//table[@class="vxe-table--body"]//tr[1]/td[4])[2]//input', '1没有数据修改')
        ele1 = self.item.get_find_element_xpath(
            '(//table[@class="vxe-table--body"]//tr[1]/td[4])[2]//input').get_attribute(
            "value")
        self.item.click_button('//div[@class="vxe-modal--footer"]//span[text()="确定"]')
        self.item.get_find_message()
        self.item.select_input('父项料号', '1没有数据修改')
        ele2 = self.item.get_find_element_xpath('(//table[@class="vxe-table--body"]//tr[1]/td[4])[1]').get_attribute(
            "innerText")
        assert ele1 == ele2
        assert not self.item.has_fail_message()
    ...
    @allure.story("模拟多选删除")
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_shiftdel(self, login_to_item):
        self.item.right_refresh('未领料明细')
        ...
        sleep(1)
        ActionChains(self.driver).key_down(Keys.CONTROL).send_keys('i').key_up(Keys.CONTROL).perform()
        self.item.click_button('(//table[@class="vxe-table--body"]//tr[1]/td[4])[2]')
        self.item.enter_texts('(//table[@class="vxe-table--body"]//tr[1]/td[4])[2]//input', '1没有数据修改1')
        self.item.click_button('(//table[@class="vxe-table--body"]//tr[2]/td[4])[2]')
        self.item.enter_texts('(//table[@class="vxe-table--body"]//tr[2]/td[4])[2]//input', '1没有数据修改12')
        sleep(1)
        ele1 = self.item.get_find_element_xpath(
            '(//table[@class="vxe-table--body"]//tr[1]/td[4])[2]').text
        ele2 = self.item.get_find_element_xpath(
            '(//table[@class="vxe-table--body"]//tr[2]/td[4])[2]//input').get_attribute("value")
        self.item.click_button('//div[@class="vxe-modal--footer"]//span[text()="确定"]')
        self.item.get_find_message()
        self.item.select_input('父项料号', '1没有数据修改1')
        ele11 = self.item.get_find_element_xpath('(//table[@class="vxe-table--body"]//tr[1]/td[4])[1]').get_attribute(
            "innerText")
        ele22 = self.item.get_find_element_xpath('(//table[@class="vxe-table--body"]//tr[2]/td[4])[1]').get_attribute(
            "innerText")
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_ctrlC(self, login_to_item):
        self.item.right_refresh('未领料明细')
        self.item.click_button('//table[@class="vxe-table--body"]//tr[2]//td[2]')
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_shift(self, login_to_item):
        elements = ['(//table[@class="vxe-table--body"]//tr[1]//td[1])[2]',
                    '(//table[@class="vxe-table--body"]//tr[2]//td[1])[2]']
        self.item.click_button(elements[0])
        # 第二个单元格 Shift+点击（选择范围）
    ...
    # @pytest.mark.run(order=1)
    def test_unpickedMaterialDetails_ctrls(self, login_to_item):
        elements = ['(//table[@class="vxe-table--body"]//tr[1]//td[1])[2]',
                    '(//table[@class="vxe-table--body"]//tr[2]//td[1])[2]']
        self.item.click_button(elements[0])
        # 第二个单元格 Shift+点击（选择范围）
